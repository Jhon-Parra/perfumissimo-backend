...
const notifyOrderCreated = async (orderId) => {
    ...
    const hasAddons = !!(order.envio_prioritario || order.perfume_lujo);
    const addonsHtml = hasAddons
        ? `
           ...
           <ul style="list-style: none; padding: 0;">
             ${(order.envio_prioritario ? `<li style="padding: 10px 0; border-bottom: 1px solid #f9f9f9; display: flex; justify-content: space-between;"><span style="color: #666;">Envío Prioritario</span> <strong style="color: #2D4C3B;">${escapeHtml(formatMoneyCop(order.costo_envio_prioritario || 0))}</strong></li>` : '')}
             ${(order.perfume_lujo ? `<li style="padding: 10px 0; border-bottom: 1px solid #f9f9f9; display: flex; justify-content: space-between;"><span style="color: #666;">Perfume de Lujo</span> <strong style="color: #2D4C3B;">${escapeHtml(formatMoneyCop(order.costo_perfume_lujo || 0))}</strong></li>` : '')}
           </ul>`
        : '';
    const subtotalHtml = order.subtotal_productos !== undefined
        ? `<div style="display: flex; justify-content: space-between; padding: 8px 0; color: #666;">
             <span>Subtotal de productos:</span> 
             <strong style="color: #2D4C3B;">${escapeHtml(formatMoneyCop(order.subtotal_productos || 0))}</strong>
           </div>`
        : '';
