...
exports.deletePromotion = exports.updatePromotionActive = exports.updatePromotion = exports.getPromotionsAdmin = exports.getPromotions = exports.createPromotion = void 0;
        ...
        try {
            ...
            if ((product_scope || 'GLOBAL') === 'SPECIFIC') {
                for (const pid of (product_ids || [])) {
                    await connection.query('INSERT INTO PromocionProductos (promocion_id, producto_id) VALUES ($1, $2) ON CONFLICT DO NOTHING', [id, pid]);
                }
            }
            if ((audience_scope || 'ALL') === 'CUSTOMERS') {
                for (const uid of (audience_user_ids || [])) {
                    await connection.query('INSERT INTO PromocionUsuarios (promocion_id, usuario_id) VALUES ($1, $2) ON CONFLICT DO NOTHING', [id, uid]);
                }
            }
    ...
    try {
        ...
        try {
            ...
            if (imagen_url !== undefined) {
                updates.push('imagen_url = $' + (params.length + 1));
            ...
            if (fecha_inicio !== undefined) {
                updates.push('fecha_inicio = $' + (params.length + 1));
            ...
            if (product_scope !== undefined) {
                updates.push('product_scope = $' + (params.length + 1));
            ...
            if (audience_scope !== undefined) {
                updates.push('audience_scope = $' + (params.length + 1));
                params.push(audience_scope);
            }
            if (audience_segment !== undefined) {
                updates.push('audience_segment = $' + (params.length + 1));
...
const updatePromotionActive = async (req, res) => {
    try {
        ...
        const [result] = await database_1.pool.query('UPDATE Promociones SET activo = $1 WHERE id = $2', [!!activo, id]);
...
const deletePromotion = async (req, res) => {
    try {
        const { id } = req.params;
        const result = await database_1.pool.query(`DELETE FROM Promociones WHERE id = $1`, [id]);
        if (result?.rowCount === 0) {
            res.status(404).json({ error: 'Promocion no encontrada' });
            return;
        }
